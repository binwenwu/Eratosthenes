...
{
	FILE* pFile = fopen("../../../result/result_Bcast.txt", "a+");
...
int main(int argc, char* argv[]) {
	...
	long long int low_index;          /* Lowest index on this proc */
	long long int low_value;          /* Lowest value on this proc */
	long long int high_index;         /* Highest index on this proc */
	long long int high_value;         /* Highest value on this proc */
	long long int n;                  /* Sieving from 2, ..., 'n' */
	int p;                  /* Number of processes */
	long long int proc0_size;         /* Size of proc 0's NewArray */
	long long int prime;              /* Current prime */
	long long int size;               /* Elements in 'marked' */
	char* NewMarked;       /*portion of 2...(in newarray)*/
	int low_NewArray_index;      /* Lowest index on NewArray*/
	int low_NewArray_value;      /* Lowest valuue on NewArray*/
	int high_NewArray_index;     /* highest index on NewArray*/
	int high_NewArray_value;     /* highest value on NewArray*/
	...
	广播优化
	*/
	//先找前sqrt(n)内的素数,再通过这些素数筛选后续素数
	int sqrt_N = (((int)sqrt((double)n)) - 1) / 2;

	low_NewArray_value = MIN(0, sqrt_N % p) * 2 + 3; //进程的第一个数
	high_NewArray_value = ((sqrt_N / p) + MIN(1, sqrt_N % p)) * 2 + 1;//进程的最后一个数

	// Bail out if all the primes used for sieving are not all held by process 0
	proc0_size = (sqrt_N - 1) / p;


	// 如果有太多进程
	if ((2 + proc0_size) < (int)sqrt((double)sqrt_N)) {
		if (!id) printf("Too many processes \n");
		MPI_Finalize();
		exit(1);
	}
	...
	do {
		// 从小数组开始标只会命中第一个条件
		first = (prime * prime - low_NewArray_value) / 2;

		// 从第一个素数开始，标记该素数的倍数为非素数
		for (int i = first; i < sqrt_N; i += prime) {
			NewMarked[i] = 1;
		}
